...
# ...
from env.market_debugged import MarketState, Order, Trade
...
class Episode:
    ...
    def __iter__(self):
        ...
        for step, timestamp, *monitor_state in self._data_monitor.itertuples():
            ...
            self._timestamp_next = self._data_monitor.iloc[min(step + 1, len(self._data_monitor.index)-1), 0]
            ...
            # info
            logging.info("(INFO) step {step}, progress {progress}, eta {eta}, time {eta}".format(
                step=step,
                progress=progress,
                eta=eta,
            ))
...
class Backtest:
    ...
    def _market_step(self, market_id, book_state, trades_state):
        ...
        self.market_state_list[market_id].update(
            book_update=book_state,
            trade_update=trades_state,
        )
    ...
    def run_generator(self,
        identifier_list:list,
        date_start:str=pd.Timestamp("2016-02-01"),
        date_end:str=pd.Timestamp("2016-02-04"),
        ...
        if episode_interval:
            ...
            # build episode_grid
            episode_grid = pd.date_range(start=date_start, end=date_end + pd.Timedelta("1d"),
